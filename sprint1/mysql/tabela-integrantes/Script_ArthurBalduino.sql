...
SELECT 
    ...
    CONCAT('CPF: ', CPF) AS "Documento",
    CONCAT(c.email, ' | Tel: ', telefone) AS "Contato",
    CONCAT('CEP: ', CEP) AS "Endereço"
FROM cliente;

SELECT
    id_plantacao AS "ID Plantação",
    nome_lote AS "Lote",
    CONCAT(area_m2, ' m²') AS "Área Plantada",
    data_plantio AS "Data Plantio",
    data_colheita AS "Data Colheita",
    CASE
        WHEN data_colheita IS NULL THEN 'Em Produção'
        ELSE 'Colhido'
    END AS "Status da Plantação"
    FROM plantacao;
    
SELECT
    id_sensor AS "ID Sensor",
    CONCAT('Unidade: ', unidade) AS "Unidade de Medida",
    data_leitura AS "Data da Leitura",
    CASE
        WHEN valor = 1 THEN 'Sensor Ativo'
        WHEN valor = 0 THEN 'Sensor Inativo'
